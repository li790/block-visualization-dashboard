...
def create_fee_items_bar_chart(data, month):
    """
    创建二级费项累计对比柱状图，根据选择的月份显示各费项的月累目标金额和月累已发生金额
    ...
    month_index = month - 1  # 转换为0-based索引
    ...
    # 根据费项数量动态调整横坐标字体角度
    tick_angle = 45 if len(names) > 10 else 0
    
    # 计算合适的Y轴刻度
    max_value = max(max(targets) if targets else 0, max(actuals) if actuals else 0)
    if max_value > 0:
        tick_step = max_value / 5
        tick_vals = [i * tick_step for i in range(6)]
    else:
        tick_vals = [0, 100000, 200000, 300000, 400000, 500000]
    
    fig.update_layout(
        title={
            'text': f"二级费项累计对比 ({month}月)",
            'x': 0.5,
            'xanchor': 'center',
...
def create_bar_chart(data):
    ...
    if isinstance(data, dict):
        # 处理字典类型数据 (二级费项累计对比)
        if not data.get('fee_items'):
            # 如果没有fee_items，尝试使用cum_target和cum_actual创建简单的折线图
            if data.get('cum_target') and data.get('cum_actual'):
        ...
        # 计算月累序列（一次累加）
        cum_actual = np.cumsum(data['已发生成本'].values.astype(float))
        cum_target = np.cumsum(data['目标成本'].values.astype(float))
        ...
        # 根据月份数量动态调整横坐标字体角度（月份通常不会超过12个，所以保持0度）
        tick_angle = 0  # 月份图表通常不需要旋转
        
        # 设置双Y轴：左侧金额（柱），右侧累计金额（折线）
        # 设置双Y轴：左侧金额（柱），右侧累计金额（折线）
        fig.update_layout(
            title={
                'text': "每月费项成本与月累趋势",
...
def create_project_comparison_chart(all_data, month):
    ...
    month_cum_targets = [ (all_data[p]['cum_target'][month_index] / 10000) if month_index < len(all_data[p]['cum_target']) else 0 for p in projects]
    month_cum_actuals = [ (all_data[p]['cum_actual'][month_index] / 10000) if month_index < len(all_data[p]['cum_actual']) else 0 for p in projects]
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate='<b>%{x}</b><br>年总目标: %{y:,.2f} 万元<extra></extra>',
        yaxis='y'
    ))
    
    # 添加月累目标折线
    fig.add_trace(go.Scatter(
        x=projects, y=month_cum_targets, mode='lines+markers', 
        name='月累目标(万元)', 
        line=dict(color='#e53935', width=3, dash='solid'),
        marker=dict(size=10, color='#e53935'),
        hovertemplate='<b>%{x}</b><br>月累目标: %{y:,.2f} 万元<extra></extra>',
        yaxis='y'
    ))
    # 添加月累已发生折线
    fig.add_trace(go.Scatter(
        x=projects, y=month_cum_actuals, mode='lines+markers', 
        name='月累已发生(万元)', 
        line=dict(color='#43a047', width=3, dash='solid'),
        marker=dict(size=10, color='#43a047'),
        hovertemplate='<b>%{x}</b><br>月累已发生: %{y:,.2f} 万元<extra></extra>',
        yaxis='y'
    ))
    
    # 根据项目数量动态调整横坐标字体角度
    tick_angle = 45 if len(projects) > 10 else 0
    
    # 依据所选月份动态设置标题并根据数据设置金额轴范围
    max_money = 0
    if month_cum_targets:
        max_money = max(max_money, max(month_cum_targets))
    if month_cum_actuals:
        max_money = max(max_money, max(month_cum_actuals))
    money_margin = max_money * 0.15 if max_money > 0 else 1

    fig.update_layout(
        title={
            'text': f"项目使用率与月累金额对比（{int(month)}月）",
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 18, 'family': 'Microsoft YaHei'}
        },
        ...
        yaxis=dict(
            ...
            side='left',
            rangemode='tozero',
            range=[0, max_money + money_margin] if max_money > 0 else None
        ),
        yaxis2=dict(
    ...
    # 统一hover数值格式
    fig.update_traces(selector=dict(type='bar'), hovertemplate='<b>%{x}</b><br>%{y:.1f}%<extra></extra>')
    return fig

def create_kpi_display(all_data, month=None):
    """创建KPI指标显示，按年使用率从高到低排序"""
    kpi_data = []
    month_index = int(month) - 1 if month else None
    for project_name, data in all_data.items():
        month_cum_target_wy = round((data['cum_target'][month_index] / 10000), 2) if month_index is not None else None
        month_cum_actual_wy = round((data['cum_actual'][month_index] / 10000), 2) if month_index is not None else None
        kpi_data.append({
            "项目": project_name,
            "年总目标(万元)": data['year_cum_target_wy'],
            "月累目标(万元)": month_cum_target_wy if month_index is not None else data['cum_target'][-1]/10000,
            "月累已发生(万元)": month_cum_actual_wy if month_index is not None else data['cum_actual'][-1]/10000,
    ...
    # 按年使用率从高到低排序
    kpi_data.sort(key=lambda x: x["年使用率(%)"], reverse=True)
    return kpi_data 
...
def create_three_donut_charts(data, month):
    ...
    if isinstance(month, str):
        ...
        month_int = month
    time_progress = data.get('time_progress', (month_int / 12) * 100)  # 时间进度
...
def create_secondary_fee_combined_chart(secondary_fee_data, selected_fee_name):
    """创建二级费项组合图表：柱状图显示单月数据，折线图显示累计数据（合并为一张图表）
    
    Args:
        secondary_fee_data: 二级费项月度数据列表
        selected_fee_name: 选中的费项名称
        
    Returns:
        plotly.graph_objects.Figure: 组合图表
    """
    # 查找选中的费项数据
    selected_data = None
    for item in secondary_fee_data:
        if item['name'] == selected_fee_name:
            selected_data = item
            break
    ...
    # 单位换算：元 -> 万元
    target_wan = [float(x) / 10000.0 for x in selected_data['target']]
    actual_wan = [float(x) / 10000.0 for x in selected_data['actual']]
    cum_target_wan = [float(x) / 10000.0 for x in selected_data['cum_target']]
    cum_actual_wan = [float(x) / 10000.0 for x in selected_data['cum_actual']]

    # 为0值提供极小显示值，保证在图上可见（不影响悬浮和文本显示）
    def display_vals(values):
        eps = 1e-6
        return [v if v != 0 else eps for v in values]

    def text_vals(values):
        return [f"{v:.0f}" if abs(v) >= 0.005 else "0" for v in values]
    ...
    fig.add_trace(
        go.Bar(
            name='单月目标成本',
            x=months,
            y=display_vals(target_wan),
            marker_color='#1f77b4',
            opacity=0.8,
            customdata=target_wan,
            text=text_vals(target_wan),
            textposition='outside',
            cliponaxis=False,
            hovertemplate='月份=%{x}<br>单月目标成本=%{customdata:.2f} 万元<extra></extra>'
        )
    )
    ...
    fig.add_trace(
        go.Bar(
            name='单月已发生成本',
            x=months,
            y=display_vals(actual_wan),
            marker_color='#ff7f0e',
            opacity=0.8,
            customdata=actual_wan,
            text=text_vals(actual_wan),
            textposition='outside',
            cliponaxis=False,
            hovertemplate='月份=%{x}<br>单月已发生成本=%{customdata:.2f} 万元<extra></extra>'
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            name='月累目标',
            x=months,
            y=display_vals(cum_target_wan),
            mode='lines+markers',
            line=dict(color='#d62728', width=3),
            marker=dict(size=6),
            yaxis='y1',
            customdata=cum_target_wan,
            hovertemplate='月份=%{x}<br>月累目标=%{customdata:.2f} 万元<extra></extra>'
        )
    )
    ...
    fig.add_trace(
        go.Scatter(
            name='月累已发生',
            x=months,
            y=display_vals(cum_actual_wan),
            mode='lines+markers',
            line=dict(color='#2ca02c', width=3),
            marker=dict(size=6),
            yaxis='y1',
            customdata=cum_actual_wan,
            hovertemplate='月份=%{x}<br>月累已发生=%{customdata:.2f} 万元<extra></extra>'
        )
    )
    ...
    fig.update_layout(
        title={
            'text': f'{selected_fee_name} - 整体成本分析',
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 18, 'color': '#1D1D1F'}
        },
